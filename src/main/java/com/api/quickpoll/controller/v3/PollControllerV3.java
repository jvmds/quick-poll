...
import com.api.quickpoll.controller.v1.PollControllerV1;
...
//		poll.add(linkTo(methodOn(PollControllerV3.class).getAllPolls()).slash(poll.getPollId()).withSelfRel());
//		poll.add(linkTo(methodOn(VoteControllerV3.class).getAllVotes(poll.getPollId())).withRel("votes"));
//		poll.add(linkTo(methodOn(ComputerControllerV3.class).computeResult(poll.getPollId())).withRel("compute-result"));
	}
